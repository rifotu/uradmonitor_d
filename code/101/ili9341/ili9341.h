...
public:
	...
	/*
	 * Draw rgb16 image starting at (x,y) top left corner.  Make sure the image pointer contains width * height pixels
	 */
	void drawImageRGB16(uint16_t ltx, uint16_t lty, const uint16_t *image, uint16_t width, uint16_t height, uint8_t size);
};

...
// i2c write
void BME280::writemem(uint8_t reg, uint8_t value) {
	i2c_start_wait(BME280_ADDR | I2C_WRITE);
	i2c_write(reg);
	i2c_write(value);
	i2c_stop();
}
...
// i2c read
void BME280::readmem(uint8_t reg, uint8_t buff[], uint8_t bytes) {
	uint8_t i =0;
	i2c_start_wait(BME280_ADDR | I2C_WRITE);
	i2c_write(reg);
	i2c_rep_start(BME280_ADDR | I2C_READ);
	for(i=0; i<bytes; i++) {
		if(i==bytes-1)
			buff[i] = i2c_readNak();
		else
...
bool BME280::begin(uint8_t a) {
  ...
  uint8_t id = 0;
  readmem(BME280_REGISTER_CHIPID, &id, 1);
  if (id != 0x60) return false;

  getcalibration();
  writemem(BME280_REGISTER_CONTROLHUMID, 0x03); // Set before CONTROL (DS 5.4.3)
  writemem(BME280_REGISTER_CONTROL, 0x3F);
  return true;
}

void BME280::getcalibration() {
	uint8_t buffer[26];
	readmem(BME280_REGISTER_T1, buffer, 26);
	...
	readmem(BME280_REGISTER_H2, buffer, 7);
...
void BME280::readSensors(float *temperature, uint32_t *pressure, uint8_t *humidity) {
	...
	readmem(BME280_REGISTER_PRESSUREDATA, buffer, 8);

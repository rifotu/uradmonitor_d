...
bool UI::pageInit() {
	...
	m_lcd->drawString(CENTERX, textY(7,1), 1, BLACK, TRANSPARENT, STRING_NAME);
	m_lcd->drawString(CENTERX, textY(8,1), 1, BLUE, TRANSPARENT, STRING_WEB);
	m_lcd->drawString(CENTERX, textY(8,2), 2, BLACK, TRANSPARENT, STRING_SHORTNAME);

	uint8_t start = 25;
	m_lcd->drawString(CENTERX, textY(start++, 1), 1, BLACK, TRANSPARENT, STRING_SELFCHECK);
	m_lcd->drawStringF(CENTERX, textY(start++, 1), 1, PURPLE, TRANSPARENT, "DeviceID %08lX", deviceID);
	m_lcd->drawStringF(CENTERX, textY(start++, 1), 1, BLACK, TRANSPARENT, "Type:%X HW:%u SW:%u %s", DEV_MODEL, VER_HW, VER_SW, aux_detectorName(DEV_RAD_DETECTOR));
	float voltageBat = readBatVoltage();
	m_lcd->drawStringF(CENTERX, textY(start++, 1), 1, BLACK, TRANSPARENT, STRING_BATVOLTAGE1, voltageBat);
	if (voltageBat < BATTERY_LIMIT) {
		m_lcd->drawString (CENTERX, textY(start++, 1), 1, RED, TRANSPARENT, STRING_BATDISCHARGED);
		return false;
	}
	m_lcd->drawStringF(CENTERX, textY(start++, 1), 1, BLACK,	TRANSPARENT, STRING_STARTUPDONE, time.getInterval());
	return true;
}

...
** Description:	The BMP180 from Bosch is a temperature and pressure sensor with a I2C interface
**
** www.pocketmagic.net
**
...
// i2c write
void BMP180::writemem(uint8_t reg, uint8_t value) {
...
// i2c read
void BMP180::readmem(uint8_t reg, uint8_t buff[], uint8_t bytes) {
	uint8_t i =0;
	i2c_start_wait(BMP180_ADDR | I2C_WRITE);
	i2c_write(reg);
...
int32_t BMP180::readRawTemperature(void) {
	...
	writemem(BMP180_REGCONTROL, BMP180_REGREADTEMPERATURE);
	_delay_ms(5); // min. 4.5ms read Temp delay
	readmem(BMP180_REGCONTROLOUTPUT, buff, 2);
...
uint32_t BMP180::readRawPressure(int32_t rawtemperature ) {
	uint8_t buff[3];
	memset(buff, 0, sizeof(buff));
	writemem(BMP180_REGCONTROL, BMP180_REGREADPRESSURE + (BMP180_MODE << 6));
	_delay_ms(2 + (3<<BMP180_MODE));
	readmem(BMP180_REGCONTROLOUTPUT, buff, 3);
